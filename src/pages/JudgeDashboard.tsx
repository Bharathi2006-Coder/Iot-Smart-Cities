...
export const JudgeDashboard: React.FC<JudgeDashboardProps> = ({
  ...
  const [scoreInputs, setScoreInputs] = useState<
    Record<string, { quiz: string; simulation: string; saving?: boolean }>
  >({});
    ...
    try {
      ...
      data.forEach((t) => {
        initialInputs[t.id] = {
          ...
          simulation: t.iotSimulationScore !== null ? String(t.iotSimulationScore) : '',
        };
      });
  ...
  const handleScoreChange = (teamId: string, field: 'quiz' | 'simulation', value: string) => {
    setScoreInputs((prev) => ({
      ...prev,
      [teamId]: {
        ...prev[teamId],
  ...
  const handleSaveEvaluation = async (team: Team) => {
    ...
    const simNum = parseFloat(inputs.simulation);

    // Strict validation (0 to 50)
    if (isNaN(quizNum) || quizNum < 0 || quizNum > 50) {
      setNotification({
        type: 'error',
        message: `Validation error for Team ${team.teamNumber}: Technical Quiz score must be between 0 and 50.`,
      });
      return;
    }

    if (isNaN(simNum) || simNum < 0 || simNum > 50) {
      setNotification({
        type: 'error',
        message: `Validation error for Team ${team.teamNumber}: IoT Simulation score must be between 0 and 50.`,
      });
      return;
    }
      ...
      <div className="p-4 rounded-2xl bg-slate-900 text-white flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
        <div className="space-y-0.5 text-xs">
          ...
          <p className="text-slate-300">
            • <strong>Round 1 (Technical Quiz):</strong> Max 50 Marks &nbsp;|&nbsp; • <strong>Round 2 (IoT Simulation):</strong> Max 50 Marks &nbsp;|&nbsp; • <strong>Total:</strong> Max 100 Marks
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {filteredTeams.map((team) => {
            const inputs = scoreInputs[team.id] || { quiz: '', simulation: '', saving: false };
            const qVal = parseFloat(inputs.quiz) || 0;
            const sVal = parseFloat(inputs.simulation) || 0;
            const calculatedTotal = (parseFloat(inputs.quiz) >= 0 ? qVal : 0) + (parseFloat(inputs.simulation) >= 0 ? sVal : 0);
                  ...
                  <div className="p-4 rounded-2xl bg-teal-50/50 border border-teal-200/80 space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        step="0.5"
                        placeholder="0 - 50"
                        value={inputs.simulation}
                        onChange={(e) => handleScoreChange(team.id, 'simulation', e.target.value)}
                        className="w-full px-3 py-2 text-base font-bold font-mono text-slate-900 rounded-xl border border-teal-300 bg-white focus:outline-none focus:ring-2 focus:ring-teal-500"
                      />
                      <span className="text-xs font-bold text-slate-400 font-mono">/ 50</span>
                    </div>
                    <p className="text-[11px] text-slate-500">Smart city modeling &amp; automation</p>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl bg-slate-900 text-white space-y-3 flex flex-col justify-between">
                    ...
                    <div className="flex items-baseline gap-2">
                      ...
                      <span className="text-xs text-slate-400">
                        ({inputs.quiz || 0} + {inputs.simulation || 0})
                      </span>
                    </div>
