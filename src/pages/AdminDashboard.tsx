...
import { generateAttendancePDF, generateWinnersPDF } from '../utils/pdfGenerator';
import { 
  LayoutDashboard, 
  Users, 
  UserPlus, 
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const handleCreateTeam = async (e: React.FormEvent) => {
    ...
    try {
      await adminApi.addTeam({
        ...
        members: newMembers.filter((m) => m.name.trim().length > 0),
      });
  ...
  const navMenuItems: { id: AdminTab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'pdf-reports', label: 'PDF Reports', icon: <FileDown className="w-4 h-4" /> },
    ...
    <div className="min-h-[85vh] bg-slate-50 flex flex-col md:flex-row animate-in fade-in duration-200">
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-x-auto space-y-6">
            ...
            <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                >
                  ...
                  <div className="text-left">
                    <p className="font-bold">Download Official PDFs</p>
                    <span className="text-[11px] font-normal text-slate-600">Attendance &amp; Winners</span>
                  </div>
                </button>
              </div>
            </div>
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="bg-slate-900 text-white font-bold uppercase tracking-wider">
                      ...
                      <th className="py-3 px-4 text-center">Quiz (50)</th>
                      <th className="py-3 px-4 text-center">Simulation (50)</th>
                ...
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="bg-slate-900 text-white font-bold uppercase tracking-wider">
                      ...
                      <th className="py-3 px-4">Team Name</th>
                      <th className="py-3 px-4 text-center">Assigned Judge</th>
                      <th className="py-3 px-4 text-center">Technical Quiz (/50)</th>
                      <th className="py-3 px-4 text-center">IoT Simulation (/50)</th>
                      <th className="py-3 px-4 text-center">Total (/100)</th>
                      <th className="py-3 px-4 text-center">Rank</th>
                    ...
                    {teams.map((team) => (
                      <tr key={team.id} className="hover:bg-slate-50">
                        ...
                        <td className="py-3 px-4 text-center font-mono font-semibold">
                          {team.technicalQuizScore !== null ? `${team.technicalQuizScore} / 50` : '--'}
                        </td>
                        <td className="py-3 px-4 text-center font-mono font-semibold">
                          {team.iotSimulationScore !== null ? `${team.iotSimulationScore} / 50` : '--'}
                        </td>
        ...
        {activeTab === 'rankings' && (
          <div className="space-y-6 animate-in fade-in">
            ...
            <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-xs text-emerald-900 space-y-1">
              ...
              <span>1st: IoT Based Simulation score → 2nd: Technical Quiz score → 3rd: Admin resolution</span>
            </div>
                    ...
                    {teams
                      ...
                      .map((team) => (
                        <tr key={team.id} className="hover:bg-slate-50 font-medium">
                          ...
                          <td className="py-3 px-4 text-center font-mono">
                            {team.technicalQuizScore} / 50
                          </td>
                          <td className="py-3 px-4 text-center font-mono">
                            {team.iotSimulationScore} / 50
                          </td>
                          <td className="py-3 px-4 text-center font-mono font-bold text-emerald-800 text-sm">
                            {team.totalScore} / 100
                          </td>
        ...
        {activeTab === 'pdf-reports' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="border-b border-slate-200 pb-4">
              <h1 className="text-2xl font-black text-slate-900 tracking-tight">Official PDF Report Generator</h1>
              <p className="text-xs text-slate-500">
                Generate and download official, high-resolution printable reports formatted with college headers
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              
              {/* PDF 1: Attendance Report */}
              <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-xs space-y-4">
                <div className="w-12 h-12 rounded-xl bg-emerald-100 text-emerald-800 flex items-center justify-center">
                  <CalendarCheck className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-slate-900">Attendance PDF Report</h3>
                ...
                <div className="pt-2">
                  <button
                    onClick={() => {
                      if (eventSettings) generateAttendancePDF(eventSettings, teams);
                    }}
      ...
      {editingTeam && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white max-w-lg w-full p-6 rounded-3xl shadow-2xl border border-slate-200 space-y-4 animate-in zoom-in-95">
